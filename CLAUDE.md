...
- `scanOpenSpecAggregated(basePath, { aggregate })` — 跨 worktree 聚合掃描：探索同 repo 全部 worktree，active changes **依 slug 去重**（以 git 分歧選舉勝出副本：非主 worktree 只有在該 change 超前 main 的 `HEAD`／有未提交修改時才是候選，無人分歧則留在 main，多個分歧副本間以 mtime tiebreak）並附來源、archived 依 slug 去重、specs 取主 worktree；單一 worktree / 非 git / 關閉聚合時等同 `scanOpenSpec`
